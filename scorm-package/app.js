...
class PresentationApp {
  ...
  updateSCORMProgress() {
    ...
    // Enviar progreso al LMS y ubicación aproximada
    this.scormAPI.setProgress(progressPercent);

    console.log(
      `Progreso SCORM: ${progressPercent.toFixed(1)}% (${
        this.visitedSlides.size
      }/${this.totalSlides} módulos)`
    );
  ...
  bindEvents() {
    ...
    // HTML tooltips se inicializan dentro de initSmartTooltips
  }
  ...
  initSmartTooltips() {
    ...
    tooltipItems.forEach((item, index) => {
      ...
      const tooltipId =
        item.getAttribute("aria-describedby") || `tooltip-${index}`;
  ...
  initHTMLTooltips() {
    const htmlTooltips = document.querySelectorAll(
      '.tooltip-text[data-tooltip-html="true"]'
    );
        ...
        if (window.innerWidth <= 768) {
          ...
          htmlTooltips.forEach((otherTooltip) => {
            if (otherTooltip !== this) {
              const otherContent =
                otherTooltip.querySelector(".tooltip-content");
              if (otherContent) {
          ...
          const isExpanded =
            tooltipContent.classList.contains("mobile-expanded");
          if (isExpanded) {
  ...
  animateToolsSlide(slide) {
    const toolSections = slide.querySelectorAll(
      ".tool-section, .attention-item"
    );
  ...
  animateDefaultSlide(slide) {
    const animatableElements = slide.querySelectorAll(
      "h2, h3, .commitment-item, .norm-item, .question-item, .analysis-item"
    );
