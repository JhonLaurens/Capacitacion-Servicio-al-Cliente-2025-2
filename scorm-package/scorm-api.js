...
class ScormAPI {
  ...
  initialize() {
    ...
    try {
      ...
      if (result === "true") {
        ...
        this.lessonStatus =
          this.getValue("cmi.core.lesson_status") || "not attempted";
  ...
  setProgress(progressPercent) {
    ...
    // Actualizar progreso (SCORM 1.2 no soporta cmi.progress_measure)
    // Guardamos porcentaje en lesson_location para trazabilidad
    this.setValue("cmi.core.lesson_location", progressPercent.toFixed(0));
  ...
  setScore(score, maxScore = 100, minScore = 0) {
    ...
    console.log(
      `Puntuación establecida: ${this.score}/${this.maxScore} - ${
  ...
  calculateSessionTime() {
    ...
    // SCORM 1.2 session_time must be HH:MM:SS
    const pad = (n) => n.toString().padStart(2, "0");
    const sessionTimeString = `${pad(hours)}:${pad(minutes)}:${pad(seconds)}`;
